...
# -----------------------------
# Snapshot sanitation for PK-Sim
# -----------------------------
...
# ---- formulations: ensure FormulationType from examples ----
def _sanitize_formulations(result: dict, data_dir: Path):
    ...
    for i, f in enumerate(forms):
        ...
        if ft not in valid_types:
            f["FormulationType"] = _choose_formulation_type(name, valid_types)
        f.pop("Dose", None)
        f.pop("Disintegration time", None)
        forms[i] = f
    result["Formulations"] = forms

# ---- protocols: normalize to Advanced Protocol schema ----
def _sanitize_protocols(result: dict, data_dir: Path):
    ...
    form_name = (forms[0].get("Name") if forms else "Oral IR Formulation")

    if forms:
        f = forms[0]
        ft = (f.get("FormulationType") or f.get("Type") or "").strip()
        if ft not in valid_types:
            f["FormulationType"] = _choose_formulation_type(form_name, valid_types) if valid_types else "Formulation_Tablet_Weibull"
        f.pop("Type", None)
        f.pop("Dose", None)
        f.pop("Disintegration time", None)
        forms[0] = f
    ...
    # Pull simple dosing if present
    dose = None
    if isinstance(pr.get("Dosing"), list) and pr["Dosing"]:
        d0 = pr["Dosing"][0]
        if isinstance(d0, dict):
            t = d0.get("Time", {})
            a = d0.get("Amount", {})
            start = (t or {}).get("Value", 0.0)
            unit_t = (t or {}).get("Unit", "h")
            amount = a.get("Value", 0.0)
            unit_a = a.get("Unit", "mg")
            dose = {"start": float(start), "unit_t": unit_t, "amount": float(amount), "unit_a": unit_a}
            ...
            "SchemaItems": [
                {
                    ...
                    "Parameters": [
                        {"Name": "Start time", "Value": dose["start"] if dose else 0.0, "Unit": dose["unit_t"] if dose else "h"},
                        {"Name": "InputDose", "Value": dose["amount"] if dose else 200.0, "Unit": dose["unit_a"] if dose else "mg"},
...
# ---- simulations: ensure model, links, outputs ----
def _sanitize_simulations(result: dict, data_dir: Path):
    ...
    compound_names = set()
    for c in result.get("Compounds", []) or []:
        if isinstance(c, dict) and isinstance(c.get("Name"), str):
            compound_names.add(c["Name"])

    protocol_names = []
    for pr in result.get("Protocols", []) or []:
        if isinstance(pr, dict) and isinstance(pr.get("Name"), str):
            protocol_names.append(pr["Name"])
    default_protocol = protocol_names[0] if protocol_names else None

    individual_names = [
        i.get("Name")
        for i in (result.get("Individuals") or [])
    ...
    for s in sims:
        ...
        if not isinstance(comps, list) or not comps:
            if len(compound_names) == 1:
                s["Compounds"] = [{"Name": next(iter(compound_names))}]
            else:
                continue
        ...
        for comp in s["Compounds"]:
            ...
            if not isinstance(comp.get("Name"), str) or not comp["Name"].strip():
                if len(compound_names) == 1:
                    comp["Name"] = next(iter(compound_names))
            prot = comp.get("Protocol")
            if not isinstance(prot, dict) or not isinstance(prot.get("Name"), str) or not prot["Name"].strip():
                if default_protocol:
                    comp["Protocol"] = {"Name": default_protocol}
            prot2 = comp.get("Protocol", {})
            forms_list = prot2.get("Formulations") or []
            if not any(isinstance(f, dict) and f.get("Name") == form_name for f in forms_list):
                forms_list.append({"Name": form_name, "Key": "Formulation"})
            prot2["Formulations"] = forms_list
            comp["Protocol"] = prot2
            fixed.append(comp)
        s["Compounds"] = fixed

        if "OutputSchema" not in s and "Output" in s:
            s.pop("Output", None)
...
def rag_json_build(section: str, values: Dict[str, Any], out_path: str, data_dir: str) -> Dict[str, Any]:
    result = {"Version": 80, section: values}
    _sanitize_snapshot(result, Path(data_dir))  # ensure consistency even for single-section builds
    out = Path(out_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return {"out_path": str(out.resolve()), "section": section}
